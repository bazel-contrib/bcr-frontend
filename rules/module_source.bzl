...
def _compile_starlark_action(ctx):
    files = ctx.files.docs_bundle
    if not files:
        return None
...
def _module_source_impl(ctx):
    documentation_info = _compile_docs_action(ctx)
    module_info = _compile_starlark_action(ctx)
