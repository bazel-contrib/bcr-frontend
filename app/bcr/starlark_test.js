...
const FunctionReturnInfo = goog.require('proto.stardoc_output.FunctionReturnInfo');
...
const ModuleExtension = goog.require('proto.build.stack.starlark.v1beta1.ModuleExtension');
...
const RepositoryRule = goog.require('proto.build.stack.starlark.v1beta1.RepositoryRule');
...
const { generateAspectExample, generateFunctionExample, generateMacroExample, generateModuleExtensionExample, generateProviderExample, generateRepositoryRuleExample, generateRuleExample, getAttributeExampleValue, getFieldExampleValue, getParameterExampleValue } = goog.require('centrl.starlark');
...
testSuite({
    ...
    testGenerateProviderExample_noFields: function () {
        ...
        const expected = 'load("@rules_go//providers:providers.bzl", "GoInfo")\n\ninfo = GoInfo()';
        assertEquals(expected, result);
    },
    ...
    testGenerateProviderExample_withFields: function () {
        ...
        const expected = 'load("@rules_go//providers:providers.bzl", "MyInfo")\n\n' +
            'info = MyInfo(\n' +
            '    output_files = depset([]),\n' +
            '    is_enabled = True,\n' +
            ')';
    ...
    testGenerateRuleExample_simpleRule: function () {
        ...
        const expected = 'load("@rules_go//rules:rules.bzl", "go_library")\n\n' +
            'go_library(\n' +
            '    name = "go_library",\n' +
            '    srcs = [],  # required\n' +
            ')';
    ...
    testGenerateRepositoryRuleExample: function () {
        ...
        const expected = 'load("@rules_go//repo:repo.bzl", "go_repository")\n\n' +
            'go_repository(\n' +
            '    name = "go_repository",\n' +
            '    url = "",  # required\n' +
            ')';
    ...
    testGenerateFunctionExample_noParams: function () {
        ...
        const expected = 'load("@rules_python//utils:utils.bzl", "get_version")\n\nget_version()';
        assertEquals(expected, result);
    },
    ...
    testGenerateFunctionExample_withParams: function () {
        ...
        const expected = 'load("@rules_python//utils:utils.bzl", "process_module")\n\n' +
            'process_module(\n' +
            '    ctx,\n' +
    ...
    testGenerateMacroExample: function () {
        ...
        const expected = 'load("@rules_pkg//macros:macros.bzl", "pkg_tar")\n\n' +
            'pkg_tar(\n' +
            '    name = "pkg_tar",\n' +
            '    srcs = [],  # required\n' +
            ')';
    ...
    testGenerateAspectExample: function () {
        ...
        const expected = 'load("@rules_go//aspects:aspects.bzl", "go_proto_aspect")\n\n' +
    ...
    testGenerateModuleExtensionExample_noTags: function () {
        ...
        const expected = 'load("@rules_go//extensions:extensions.bzl", "go_sdk")\n\n' +
            '# In MODULE.bazel:\n' +
